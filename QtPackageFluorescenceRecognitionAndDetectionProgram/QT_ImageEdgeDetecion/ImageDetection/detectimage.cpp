...
void RemoveSmallRegion(Mat& Src, Mat& Dst, int AreaLimit, int CheckMode, int NeihborMode)
{
    ...
    if (NeihborMode == 1)
    {
        ...
        int a = 0;  // cout << "Neighbor mode: 4邻域." << endl;
...
void DetectImage(Mat img, string imgPath)
{
    ...
    Mat img_blur;
    blur(img_copy, img_blur, Size(10, 10));

    unsigned char grayThreashold = 18;

    for (int i = 0; i < img.rows; ++i)
    {
        for (int j = 0; j < img.cols; ++j)
        {
            if (img_blur.at<uchar>(i, j) > grayThreashold)
            {
    ...
    img_blur = imfill(img_blur, 1000);

    // Remove small spot
    threshold(img_blur, img_blur, 128, 255, THRESH_BINARY_INV);
    RemoveSmallRegion(img_blur, img_blur, 1000, 0, 1);
    ...
    findContours(img_blur, contours, hierarcy, RETR_EXTERNAL, CHAIN_APPROX_NONE);
